...
import { submitReview, deleteReview } from "../../api/axios";
...
function Orders() {
  ...
  const handleReturnSubmit = async (e) => {
    ...
    try {
      const { submitReturnRequest } = await import("../../api/axios");
      await submitReturnRequest(selectedOrderForReturn, returnData);
      toast.success("Return request submitted successfully!");
      setIsReturnModalOpen(false);
      dispatch(fetchOrders());
    } catch (err) {
      toast.error(err.response?.data?.error || "Error submitting return request");
    } finally {
  ...
  const openReturnModal = (orderId) => {
    ...
    setReturnData({ reason: "damaged", description: "" });
  ...
  if (error) {
    const errorMessage = typeof error === 'string' ? error : error?.detail || error?.message || "Something went wrong.";
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center px-4">
        ...
        <button
          ...
          className="mt-8 px-8 py-3 bg-gradient-to-r from-orange-400 to-purple-500 text-white font-black rounded-xl shadow-lg hover:shadow-orange-400/30 transition-all active:scale-95"
        >
          Return to Login
        </button>
      </div>
    ...
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center px-4">
        ...
        <p className="text-gray-400 font-bold text-center mt-3 max-w-sm leading-relaxed">
          Your order history is currently empty. Start shopping to fill it!
        </p>
        <button
          onClick={() => navigate("/")}
                  ...
                  >
                    <div className="border-t border-gray-50 px-8 py-10 bg-gray-50/20">
                      <div className="flex items-center gap-3 mb-6">
                        ...
                        <h3 className="text-[11px] font-black text-gray-400 uppercase tracking-[3px]">Package Contents</h3>
                      </div>
                        ...
                        <div className="flex gap-3 w-full md:w-auto">
                          ...
                          <button
                            onClick={async () => {
                              try {
                                const token = localStorage.getItem("accessToken");
                                const response = await fetch(`http://localhost:8000/invoice/${order.id}`, {
                                  headers: { 'Authorization': `Bearer ${token}` }
                                });
                                const responseText = await response.text();
                                if (!response.ok) throw new Error(responseText || "Failed to fetch invoice");
                                const blob = new Blob([responseText], { type: 'text/html' });
                                const url = window.URL.createObjectURL(blob);
                                const a = document.createElement('a');
                                a.href = url;
                                a.download = `Invoice_${order.order_number}.html`;
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="relative w-full max-w-lg bg-white rounded-[40px] shadow-2xl p-8 md:p-12 overflow-hidden">
              ...
              <div className="flex items-center justify-between pr-10">
                ...
                {newReview.isAltering && (
                  <button type="button" onClick={handleDeleteReview} className="p-3 bg-red-50 text-red-500 rounded-2xl hover:bg-red-500 hover:text-white transition-all border border-red-100 group" title="Delete Review">
              ...
              <form onSubmit={handleReviewSubmit} className="space-y-6">
                <div className="space-y-2">
                  ...
                  <input type="text" required value={newReview.name} onChange={(e) => setNewReview({ ...newReview, name: e.target.value })} className="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl font-bold focus:outline-none focus:ring-2 focus:ring-orange-400/20 transition-all" />
                </div>
                <div className="space-y-2">
                  <label className="text-[11px] font-black text-gray-400 uppercase tracking-[2px] block">Rating</label>
                  <div className="flex gap-2">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <button key={star} type="button" onClick={() => setNewReview({ ...newReview, rating: star })} className={`text-2xl transition-all ${star <= newReview.rating ? "text-yellow-400 scale-110" : "text-gray-200 hover:text-yellow-200"}`}><FaStar /></button>
                    ))}
                  </div>
                </div>
                <div className="space-y-2">
                  <label className="text-[11px] font-black text-gray-400 uppercase tracking-[2px] block">Your Comment</label>
                  <textarea required rows="4" value={newReview.comment} onChange={(e) => setNewReview({ ...newReview, comment: e.target.value })} className="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl font-bold focus:outline-none focus:ring-2 focus:ring-orange-400/20 transition-all resize-none" />
                </div>
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-4">
                    <label className="cursor-pointer group relative w-20 h-20 bg-gray-50 border-2 border-dashed border-gray-200 rounded-2xl flex items-center justify-center hover:border-orange-400 hover:bg-orange-50 transition-all overflow-hidden font-bold">
                      {newReview.imagePreview ? <img src={newReview.imagePreview} className="w-full h-full object-cover" alt="Preview" /> : <FaCamera className="text-gray-400 group-hover:text-orange-400" />}
                      <input type="file" className="hidden" accept="image/*" onChange={handleImageUpload} />
                    </label>
                    <p className="text-[11px] text-gray-400 font-bold uppercase tracking-wider">Add a photo to your review</p>
                  </div>
                </div>
                <button type="submit" disabled={isSubmitting} className={`w-full py-5 bg-gradient-to-br from-orange-400 to-purple-500 text-white rounded-[24px] font-black text-lg shadow-xl hover:-translate-y-1 transition-all active:scale-95 flex items-center justify-center gap-3 ${isSubmitting ? "opacity-70" : ""}`}>
                  {isSubmitting ? "Processing..." : (newReview.isAltering ? "Update Review" : "Submit Review")}
                </button>
              </form>
            </motion.div>
          </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="relative w-full max-w-lg bg-white rounded-[40px] shadow-2xl p-8 md:p-12 overflow-hidden">
              ...
              <form onSubmit={handleReturnSubmit} className="space-y-6">
                <div className="space-y-2">
                  <label className="text-[11px] font-black text-gray-400 uppercase tracking-[2px] block">Reason for Return</label>
                  <select value={returnData.reason} onChange={(e) => setReturnData({ ...returnData, reason: e.target.value })} className="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl font-bold focus:outline-none focus:ring-2 focus:ring-orange-400/20 transition-all">
                    {RETURN_REASONS.map(r => <option key={r.value} value={r.value}>{r.label}</option>)}
                  </select>
                </div>
                <div className="space-y-2">
                  <label className="text-[11px] font-black text-gray-400 uppercase tracking-[2px] block">Additional Details</label>
                  <textarea required placeholder="Describe why you are returning..." rows="4" value={returnData.description} onChange={(e) => setReturnData({ ...returnData, description: e.target.value })} className="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl font-bold focus:outline-none focus:ring-2 focus:ring-orange-400/20 transition-all resize-none" />
                </div>
                <button type="submit" disabled={isSubmittingReturn} className={`w-full py-5 bg-gradient-to-br from-orange-500 to-purple-600 text-white rounded-[24px] font-black text-lg shadow-xl hover:-translate-y-1 transition-all active:scale-95 flex items-center justify-center gap-3 ${isSubmittingReturn ? "opacity-70" : ""}`}>
